...
def test_base_two_Logarithm():
    ...
    assert synthetic.partial_at(variable_values, x) == approx(1.442695040888)
    # at x = 2
    variable_values = VariableValues({x: 2})
    assert z.evaluate(variable_values) == approx(1)
    ...
    assert synthetic.partial_at(variable_values, x) == approx(0.721347520444)
    # at x = 0
    variable_values = VariableValues({x: 0})
    with raises(DomainError):
        z.evaluate(variable_values)
...
def test_base_two_Logarithm_composition():
    ...
    assert z.synthetic().partial_at(variable_values, x) == approx(0.3606737602222)

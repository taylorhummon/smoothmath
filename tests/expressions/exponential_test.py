...
def test_exponential():
    ...
    assert synthetic_partial.evaluate(variable_values) == approx(math.e)
    # at x = -1
    variable_values = VariableValues({x: -1})
...
def test_base_two_Exponential():
    ...
    assert synthetic_partial.evaluate(variable_values) == approx(0.693147180559)
    # at x = 1
    variable_values = VariableValues({x: 1})
    ...
    assert synthetic_partial.evaluate(variable_values) == approx(1.386294361119)
    # at x = -1
    variable_values = VariableValues({x: -1})
    ...
    assert synthetic_partial.evaluate(variable_values) == approx(0.346573590279)

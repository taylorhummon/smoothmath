...
def test_reducing_plus_with_constants():
    ...
    assert _reduce_synthetic(z) == x + Constant(3)
...
def test_reducing_plus_with_negation():
    ...
    y = Variable("y")
    z = (Negation(y) + w) + x
    assert _reduce_synthetic(z) == w + (x + Negation(y))
...
def test_reducing_multiply_with_negation():
    ...
    assert _reduce_synthetic(z) == x * x
...
def test_reducing_multiply_with_reciprocal():
    ...
    y = Variable("y")
    z = x * (y * Reciprocal(w))
    assert _reduce_synthetic(z) == (Reciprocal(w) * x) * y


def test_reducing_nth_root_of_mth_power():
    u = Variable("u")
    z = NthRoot(NthPower(u, n = 4), n = 6)
    assert _reduce_synthetic(z) == NthPower(NthRoot(u, n = 3), n = 2)

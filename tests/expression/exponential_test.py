...
def test_Exponential():
    ...
    assert z.local_partial(x, point) == approx(math.e)
    assert global_x_partial.at(point) == approx(math.e)
    assert z.local_differential(point).component(x) == approx(math.e)
    assert global_differential.component_at(x, point) == approx(math.e)
    # at x = -1
    point = Point(x = -1)
...
def test_base_two_Exponential():
    ...
    assert z.local_partial(x, point) == approx(0.693147180559)
    assert global_differential.component_at(x, point) == approx(0.693147180559)
    # at x = 1
    point = Point(x = 1)
    ...
    assert z.local_partial(x, point) == approx(1.386294361119)
    assert global_differential.component_at(x, point) == approx(1.386294361119)
    # at x = -1
    point = Point(x = -1)
    ...
    assert z.local_partial(x, point) == approx(0.346573590279)
    assert global_differential.component_at(x, point) == approx(0.346573590279)
...
def test_base_two_Exponential_composition():
    ...
    point = Point(x = 3)
    assert z.evaluate(point) == approx(2)
    assert z.local_partial(x, point) == approx(2.77258872223)
    ...
    assert z.global_differential().component_at(x, point) == approx(2.77258872223)

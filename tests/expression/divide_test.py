...
def test_Divide():
    ...
    assert z.local_partial(y, point) == approx(-1.25)
    ...
    assert global_differential.component_at(x, point) == approx(0.5)
    assert global_differential.component_at(y, point) == approx(-1.25)
    # at (x, y) = (3, 0)
    point = Point(x = 3, y = 0)
...
def test_Divide_composition():
    ...
    assert z.local_partial(x, point) == approx(0.4)
    ...
    global_differential = z.global_differential()
    assert global_differential.component_at(x, point) == approx(0.4)
...
def test_Divide_with_constant_numerator_zero_composition():
    ...
    assert z.local_differential(point).component(y) == approx(0)
    assert z.global_differential().component_at(y, point) == approx(0)
